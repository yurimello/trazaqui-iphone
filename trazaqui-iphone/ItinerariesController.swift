...
class ItinerariesController : TableViewController {
    let childController: PlacesController = PlacesController()
    ...
    override func processChildren(indexPath: IndexPath){
        for child in children[indexPath.row].array!{
            childController.addJSONObject(child, labelName: "name", childName: "items")
        }
    }
